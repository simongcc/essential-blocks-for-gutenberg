...
class Hello_Gutenberg extends Component {
	...
	static getDerivedStateFromProps( nextProps, state ) {
		if ( ( nextProps.isPublishing || nextProps.isSaving ) && !nextProps.isAutoSaving ) {
			wp.apiRequest( { path: `/gutenberg-blocks/v1/update-meta?id=${nextProps.postId}`, method: 'POST', data: state } ).then(
				( data ) => {
					return data;
				},
				( err ) => {
	...
	render() {
		return (
			<Fragment>
				<PluginSidebarMoreMenuItem
					target="gutenberg-blocks-sidebar"
				>
					{ __( 'Hello Gutenberg' ) }
				</PluginSidebarMoreMenuItem>
				<PluginSidebar
					name="gutenberg-blocks-sidebar"
...
registerPlugin( 'gutenberg-blocks', {
	icon: 'admin-site',
	render: HOC,
} );